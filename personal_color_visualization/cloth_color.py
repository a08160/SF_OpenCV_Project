...
def remove_similar_bgr(image_path, face_bgr, threshold=80):
    # 이미지 로드 및 Lab 색공간 변환
...
df["skin_bgr"] = df["skin_bgr"].apply(
    lambda x: np.array(
        list(map(int, x.strip('[]').replace(',', '').split())), dtype=np.uint8
    ) if isinstance(x, str) else np.array([x], dtype=np.uint8)
)
# 사용 예시
for i in range(1, 1189):
    image_path = f"Noukky_image/{i}.png"  # 전신 이미지 경로
    face_bgr = df["skin_bgr"][i-1]  # 얼굴 bgr 값
    result_image = remove_similar_bgr(image_path, face_bgr)
